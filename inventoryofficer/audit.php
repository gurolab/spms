            ...
            <div class="card-header border-bottom border-gray-100">
                <div class="flex-between flex-wrap gap-8">
                    <div>
                        ...
                        <p class="text-gray-200 text-13 mb-0">Review actions across the system. Use View Trail to inspect field-level changes.</p>
                    </div>
                </div>
            </div>
...
(function () {
    ...
    function renderMetadata(metadata) {
        ...
        for (const key of Object.keys(metadata)) {
            ...
            rows += '<tr><th class="text-gray-500 fw-medium text-13 w-200">' + key + '</th><td>' + formatted + '</td></tr>';
        }

        return '<div class="table-responsive"><table class="table table-borderless align-middle mb-0"><tbody>' + rows + '</tbody></table></div>';
    }
    ...
    function renderTrail(trail) {
        ...
        const rows = trail.map(entry => {
            ...
            const oldValue = entry.old_value === null ? '<span class="text-gray-500">null</span>' : '<code>' + escapeHtml(entry.old_value) + '</code>';
            const newValue = entry.new_value === null ? '<span class="text-gray-500">null</span>' : '<code>' + escapeHtml(entry.new_value) + '</code>';
            return '<tr><td class="text-gray-900">' + fieldName + '</td><td class="text-gray-900">' + oldValue + '</td><td class="text-gray-900">' + newValue + '</td></tr>';
        }).join('');

        return '<div class="table-responsive"><table class="table table-striped align-middle"><thead><tr><th class="text-gray-300 text-13">Field</th><th class="text-gray-300 text-13">Old Value</th><th class="text-gray-300 text-13">New Value</th></tr></thead><tbody>' + rows + '</tbody></table></div>';
    }
